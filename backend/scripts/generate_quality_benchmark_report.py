...
def render_markdown(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"| `min_content_marker_pass_share` | `{_format_percent(sections['min_content_marker_pass_share'])}` |",
        f"| `quality_score_mean` | `{_format_percent(sections['quality_score_mean'])}` |",
        f"| `quality_pass_share` | `{_format_percent(sections['quality_pass_share'])}` |",
        "",
        "## 6. Интерпретация",
        "",
