...
Текущие улучшения document pipeline:

- PDF без текстового слоя прогоняется через локальный OCR с настраиваемым лимитом страниц и улучшенным preprocessing.
- ZIP проверяется по настраиваемым лимитам числа файлов, размера вложения, общего объёма и глубины вложенности.
- XML с недопустимыми управляющими символами может быть восстановлен после очистки, но помечается как `requires_review`.
- GGE без ZIP-структуры не считается полноценно распарсенным, но из него извлекаются читаемые строки для evidence preview.
- DOC сначала обрабатывается локальными конвертерами `antiword`/`LibreOffice`, затем fallback-извлечением читаемых строк.
