...
- актуальная продуктовая версия публикуется из ветки `main`
- дата актуализации опубликованного состояния: `2026-06-02`
- `MVP 1` завершён
...
- расширенную обработку `DOC`, `HTML/HTM`, `RTF`, `XML`, `ZIP`, `SIG`, `P7S`, `GGE`, `JPG/PNG/TIFF/BMP` в рамках текущего document pipeline
...
Это подтверждает, что базовый OCR-контур уже рабочий. В текущем pipeline OCR дополнительно использует несколько `psm`-режимов, preprocessing изображения и настраиваемые лимиты страниц, но полноценный layout-aware vision-контур остаётся задачей `MVP 2+`.
